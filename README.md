...
### 🔴 그리퍼 action 완료(drl) 26ms — DRL 실행 완료가 아님

| 항목 | 내용 |
|------|------|
| **증상** | `gripper action 완료: 완료(drl)` 가 26ms 만에 반환됨 |
| **원인** | `DrlStart.call_async()` 는 컨트롤러가 DRL 스크립트를 **수락**하면 즉시 success=True 반환. 그리퍼가 실제로 움직이는 건 그 이후 비동기 실행 |
| **영향** | action result가 돌아와도 그리퍼가 아직 물리적으로 동작 중 → 다음 MoveL이 너무 빨리 시작될 수 있음 |
| **해결 방향** | ① `command_transport=tcp` 으로 전환 + binary 프로토콜 state 스트림으로 pulse 도달 확인 (권장) ② DRL 모드 유지 시 `_execute_callback` 에서 `/gripper/state` pulse 폴링 추가 |

### 🟡 toolbox_seq_runner — action wait_for_server ROS2 호환 이슈 (수정 완료)

| 항목 | 내용 |
|------|------|
| **증상** | step 1 GRIP은 action 성공, step 2/3 MoveJ/MoveL 이후 step 4 GRIP에서 `action 서버 응답 없음` → topic fallback |
| **원인** | `rclpy.spin_until_future_complete(self, fut)` 가 내부에서 별도 executor를 돌리면서 action client 서버 발견 상태 초기화 (ROS2 known issue) |
| **수정** | `wait_for_server` 를 `__init__` 에서 1회만 호출 → `_gripper_action_ready` 플래그로 이후 grip은 재탐색 없이 goal 전송 (`cad7aa0`) |
| **검증 필요** | 수정 후 4종 시퀀스 재실행으로 모든 GRIP 스텝이 action으로 완료되는지 확인 |

### 🟡 DRL 모드에서 그리퍼 완료 판정 불확실

| 항목 | 내용 |
|------|------|
| **증상** | DRL 모드 grip action이 물리 완료 전 result 반환 → 시퀀스 타이밍 어긋날 가능성 |
| **임시 대응** | `command_transport=drl` + `time.sleep` fallback 구조 유지 중 |
| **근본 해결** | TCP 모드 전환 후 binary STATE 스트림(`cur`, `pos` 실시간 수신)으로 파지 완료 판정 |
