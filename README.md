...
| 카메라 (탑뷰) | Intel RealSense D455f (eye-to-hand, 탑뷰 고정 · scene context) |
...
| 항목 | 변경 전 | 변경 후 | 사유 |
|------|---------|---------|------|
| 공구 종류 | 플레이스홀더 3종 | **실사용 5종 확정** | 팀 협의 완료 |
| 공구함 구조 | 고정 케이스 슬롯 | **통 안에 무작위 배치** | 운영 시나리오 변경 |
| YOLO 모델 | YOLOv8 | **YOLOv11s** | 소형 객체 어텐션 + mAP +2.1%p |
| 탑뷰 카메라 역할 | YOLO 추론 | **scene context 전담** | 90cm 고도에서 소켓 11px → 검출 불가 |
| YOLO 카메라 | D455f | **Logitech C270 (그리퍼)** | 그리퍼 접근 시 근거리 → 충분한 해상도 |

#### 확정된 5종 공구 (tool_id)

| tool_id | 설명 | 슬롯 |
|---------|------|------|
| `socket_19mm` | 19mm 소켓 | (0, 0) |
| `ratchet_handle` | 래칫 핸들 | (0, 1) |
| `hex_key_set_folding` | 접이식 육각렌치 세트 | (0, 2) |
| `screwdriver_flat_6x100` | 일자 드라이버 6×100mm | (1, 0) |
| `cutter_knife` | 커터칼 | (1, 1) |

#### 완료 항목

| 항목 | 파일 | 비고 |
|------|------|------|
| 공구 카탈로그 갱신 | `config/toolbox.yaml` | 5종 슬롯 배치 + 추정 치수 |
| YOLO 클래스 갱신 | `config/vision.yaml` | class_names 인덱스 0–4 |
| 데이터셋 설정 | `datasets/tools/data.yaml` | nc=5, train/val 경로 |
