...
## 핸드오버(feat/handover) Python 환경 설치

> `hand_node`, `hand_viz_node` 실행에 필요한 패키지. **버전 조합이 맞지 않으면 MediaPipe가 NumPy 2.x와 충돌해 import 단계에서 죽는다.**

### 검증된 버전 조합 (2026-06-21 기준, Python 3.10)

| 패키지 | 검증 버전 | 주의사항 |
|--------|-----------|----------|
| `mediapipe` | **0.10.21** | 0.10.x 이상은 NumPy 2.x 미지원 — **반드시 1.x 고정** |
| `numpy` | **1.26.4** | 2.x 금지. mediapipe 0.10.x가 numpy C API 1.x 기준으로 빌드됨 |
| `opencv-python` | **4.11.0.86** | `opencv-python-headless`와 **동시 설치 금지** (심볼 충돌) |
| `scipy` | **1.15.3** | hand_node rz 계산(`Rotation` 클래스) 사용 |
| `pyrealsense2` | **2.57.7.10387** | RealSense SDK 버전과 반드시 맞춰야 함 (아래 참고) |
| `protobuf` | **4.25.9** | mediapipe 내부 의존. 5.x 올리면 mediapipe 깨짐 |
...
> `pip install pyrealsense2`는 **RealSense SDK(librealsense2) 버전과 맞춰야** 한다.
> 검증 환경: librealsense2 2.57.x + `pyrealsense2==2.57.7.10387`
> SDK 버전 확인: `rs-fw-update -l` 또는 `dpkg -l | grep librealsense`

### 자주 발생하는 오류

| 오류 메시지 | 원인 | 해결 |
|-------------|------|------|
| `A module that was compiled using NumPy 1.x cannot be run in NumPy 2.x` | numpy 2.x 설치됨 | `pip install "numpy==1.26.4"` |
| `ImportError: libGL.so.1` | opencv headless 설치 필요 | `pip install opencv-python-headless` (또는 `sudo apt install libgl1`) |
| `ImportError: ... protobuf` | protobuf 5.x | `pip install "protobuf==4.25.9"` |
| `pyrealsense2` 버전 불일치 | SDK vs pip 버전 차이 | SDK 버전 확인 후 동일 버전 pip 설치 |
...
## 현재 미해결 이슈 (2026-06-19)

### 🔴 depth_rx=0 — hand_node depth 미수신 (feat/handover)

> 브랜치: `feat/handover` | 담당: vision팀

| 항목 | 내용 |
|------|------|
| **증상** | `hand_rx` 는 정상 증가, `depth_rx=0` 고정 → `[NO_DEPTH]` 무한 반복 |
| **현재까지 시도** | QoS RELIABLE+TRANSIENT_LOCAL → BEST_EFFORT(sensor_data) 수정 (`5c29233`) |
| **현재까지 시도** | 토픽명 `/d455f/aligned_depth_to_color/…` → `/d455f/d455f/…` 수정 (mediapipe와 동일) |
| **현재까지 시도** | color 프로파일 640×480 → 1280×720 복원 (camera_info.yaml·ROI 기준 통일) |
| **미진단** | 실제 depth 토픽이 발행되는지 확인 안 됨 (카메라 "Frames didn't arrived" 경고 지속) |

**이어서 할 진단 명령어 (3개 결과를 확인해야 원인 특정 가능):**

```bash
# depth 토픽이 실제로 존재하는지
ros2 topic list | grep -i depth

# 토픽 QoS Publisher 설정 확인
ros2 topic info /d455f/d455f/aligned_depth_to_color/image_raw --verbose

# 실제 메시지 수신 여부
ros2 topic hz /d455f/d455f/aligned_depth_to_color/image_raw
```
...
| 가능성 | 확인 방법 | 조치 |
|--------|----------|------|
| USB 2.0 연결 (대역폭 부족) | `lsusb \| grep Intel` — Bus 001=USB2, Bus 002=USB3 | USB 3.0 포트로 직결, 허브 제거 |
| depth 토픽 자체가 없음 | `ros2 topic list` 결과에 depth 없으면 | 카메라 재연결 후 `realsense_bringup` 재기동 |
| 토픽명 다름 | `ros2 topic list` 에서 실제 이름 확인 | `hand_node.py:205` 토픽명 교정 |

**hand_node 실행 시 주의: 반드시 프로젝트 루트에서 실행**

```bash
cd ~/Final_project          # config/ 상대경로 기준
source ros2_ws/install/setup.bash
ros2 run vision hand_node
```

### 🟡 ROI 좌표 실측 미완료 (feat/handover)

`config/handover.yaml` ROI (`roi_x_min~max`, `roi_y_min~max`)는 placeholder 값.
depth 이슈 해결 후 실제 작업 위치에서 손 위치 확인 후 조정 필요.
...
### 2026-06-19 — place_on_hand 핸드오버 구현 + depth 디버깅 (feat/handover)

> 브랜치: `feat/handover` | 커밋: `5c29233`

#### 완료 항목

| 항목 | 내용 |
|------|------|
| **place_on_hand unit action + BT 노드** | 사람 손에 직접 공구 전달하는 동작 구현 (`8724be0`) |
| **hand_node 구현** | MediaPipe 랜드마크 + D455f aligned depth → `/hand/pose` + `/hand/ready` 발행 |
| **hand_viz_node** | ROI 박스 항상 표시, OUTSIDE_ROI 디버그 메시지 추가 |
| **depth QoS 수정** | `_QOS_DEPTH` RELIABLE+TRANSIENT_LOCAL → `sensor_data`(BEST_EFFORT) — RealSense 이미지 토픽 발행 QoS와 일치 |
| **depth 토픽명 수정** | `/d455f/aligned_depth_to_color/…` → `/d455f/d455f/aligned_depth_to_color/…` (camera_namespace=d455f + camera_name=d455f 기준, mediapipe와 동일) |
| **해상도 통일** | color 프로파일 640×480 → 1280×720 복원. depth는 640×480 유지 (848×480 초기화 실패 대응). aligned_depth는 color 해상도 따라감 |
| **Mediapipe 서브모듈 기본값 반영** | image_topic `/d455f/d455f/color/image_raw`, confidence 0.95→0.7, flip_image false |

#### 파일별 변경 내역

| 파일 | 변경 내용 |
|------|-----------|
| `ros2_ws/src/vision/vision/hand_node.py` | QoS 수정, 토픽명 수정, ApproximateTimeSynchronizer → 개별 구독 + depth 캐시 패턴 |
| `ros2_ws/src/vision/launch/realsense_bringup.launch.py` | color 1280×720 복원, depth 640×480 유지 |
| `config/handover.yaml` | ROI 주석 해상도 명확화 (color 1280×720 기준) |
| `ros2_ws/src/Mediapipe-Hand-ROS` | 서브모듈 — 기본값 파라미터 d455f 환경 맞춤 |

#### 실행 순서

```bash
# Terminal 0: 빌드 (변경 후 최초 1회)
cd ~/Final_project/ros2_ws
colcon build --packages-select vision handpose_ros
...
# Terminal 2: MediaPipe
cd ~/Final_project/ros2_ws && source install/setup.bash
ros2 run handpose_ros mediapipe_hands_node \
  --ros-args -p image_topic:=/d455f/d455f/color/image_raw

# Terminal 3: hand_node (반드시 프로젝트 루트에서 — config/ 상대경로)
cd ~/Final_project
source ros2_ws/install/setup.bash
ros2 run vision hand_node

# Terminal 4: 디버그
ros2 topic echo /hand/debug
```
