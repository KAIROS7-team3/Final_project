...
### 2026-06-03 — DSR 에뮬레이터 virtual 모드 검증 + RViz 파이프라인 확인 (역할 B)
...
| **버그 발견 및 원인 확정** | `gripper_node` DRL 초기화(`flange_serial_open`)가 virtual 에뮬레이터를 블로킹 → joint 이동 불가 원인 |

#### 확정된 virtual 테스트 방법

```bash
# virtual 모드 (기본값) — gripper_node가 DRL 초기화를 자동 생략
ros2 launch motion bringup_e0509_with_gripper.launch.py

# 관절 이동 (degree 단위)
ros2 service call /dsr01/motion/move_joint dsr_msgs2/srv/MoveJoint \
  "{pos: [30.0, 0.0, 90.0, 0.0, 90.0, 0.0], vel: 30.0, acc: 30.0, time: 0.0, radius: 0.0, mode: 0, blend_type: 0, sync_type: 1}"
```

> `mode:=virtual`(기본값)이면 gripper_node가 `flange_serial_open` 초기화를 건너뛴다.
> 실물 로봇 연결 시에만 `mode:=real host:=<IP>`를 명시한다.
...
| ~~3~~ | ~~`gripper_node` virtual 모드 대응~~ | ✅ **완료** — `mode` 파라미터로 virtual 감지, DRL 초기화 생략 |
