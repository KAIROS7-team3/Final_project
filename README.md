...
### 🔴 flange_serial DRL — 컨트롤러 모드 제약으로 그리퍼 물리 동작 불가

| 항목 | 내용 |
|------|------|
| **증상** | `gripper action 완료(drl)` / `direct cmd(drl): ok=False` 이지만 그리퍼 실제 미동작 |
| **원인** | Auto 모드: `DrlStart` 수락되나 내부 `flange_serial_open` 실행 시 alarm 2007 (`this command can only be used in manual mode`). Manual 모드: `DrlStart` 자체가 거부 (`auto mode required`). 캐치-22 상황 |
| **확인된 사항** | `SetRobotMode` ROS2 서비스로 모드 전환 가능하나 두 모드 모두 flange_serial 차단됨 |
| **근본 해결** | 컨트롤러 firmware/설정 레벨에서 flange_serial 허용 설정 필요 (또는 PC↔그리퍼 직접 RS485 연결) |
| **현재 대응** | `toolbox_seq_runner` GRIP step은 topic publish + sleep(0.5s) fallback으로 동작 중 (팔 시퀀스는 정상) |

### ✅ 4종 서랍 시퀀스 실물 검증 완료 (`383325b`)

| 시퀀스 | 결과 |
|--------|------|
| `open_0` (1층 열기) | ✅ 팔 동작 정상 |
| `close_0` (1층 닫기) | ✅ 팔 동작 정상 |
| `open_1` (2층 열기) | ✅ 팔 동작 정상 |
| `close_1` (2층 닫기) | ✅ 팔 동작 정상 |

- 그리퍼 GRIP step은 topic publish fallback으로 실행 (물리 파지는 flange_serial 이슈로 미동작)
- MoveJ/MoveL 시퀀스 전체 정상 동작 확인
...
| **seq_runner topic fallback 복귀** | action client → topic publish + sleep(0.5s) 방식으로 롤백 (flange_serial DRL 차단 우회) |
| **flange_serial 원인 규명** | Auto 모드: alarm 2007 (`manual mode required`). Manual 모드: DrlStart 거부 (`auto mode required`). 캐치-22 확인 |

#### 미해결 — flange_serial DRL 차단

컨트롤러가 Auto 모드에서 `flange_serial_open` DRL 명령을 차단함. `SetRobotMode` 서비스로 Manual 전환 시 DrlStart 자체가 거부됨. 팔 시퀀스(MoveJ/MoveL)는 ros2_control 경로라 영향 없음.
...
| 4 | flange_serial 차단 해결 (컨트롤러 설정 또는 직접 RS485 연결) | 그리퍼 물리 동작 필수 |
