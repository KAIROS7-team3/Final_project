...
### 🔴 DRL 모드 그리퍼 action 26ms 완료 — 물리 동작 완료가 아님
...
| **근본 해결** | `command_transport=tcp` 전환 + binary STATE 스트림으로 pulse 도달 확인 (권장) |
| **임시 대응** | DRL 모드 유지 시 `_execute_callback`에서 `/gripper/state` pulse 폴링 추가 필요 |

### 🟡 toolbox_seq_runner — action wait_for_server ROS2 이슈 (수정 완료, 검증 필요)

| 항목 | 내용 |
|------|------|
| **증상** | step 1 GRIP action 성공 → step 2/3 MoveJ/MoveL 후 step 4 GRIP에서 `action 서버 응답 없음` → topic fallback |
| **원인** | `rclpy.spin_until_future_complete` 가 내부 executor 돌리면서 action client 서버 발견 상태 초기화 (ROS2 known issue) |
| **수정** | `wait_for_server` 를 `__init__` 1회만 호출, `_gripper_action_ready` 플래그로 이후 goal 전송 (`cad7aa0`) |
| **검증 필요** | 수정 후 4종 시퀀스 재실행 → 모든 GRIP 스텝이 action으로 완료되는지 확인 |
...
- TW SubRoutine: `gripper_release=450`, `gripper_grap_boxhand=600` (4개 파일 전부 동일)
- `toolbox_seq_runner._grip()`은 `toolbox_motion.py` 상수(450/600)를 직접 써서 시퀀스는 맞게 동작했으나
- `action _execute_callback`이 yaml `pulse_closed_preset=700`을 참조해 700으로 닫히던 문제 수정
