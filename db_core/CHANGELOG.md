...
### Added
- **(issue #44, Option B) `ToolRepository.check_drawer_feasibility(intent, layer_id)` 추가** — 서랍(layer) 단위 DB Gate. `fetch`/`return`은 공구 단위 전체 시퀀스이고 `check_feasibility(intent, tool_id)`로 게이트하지만, `open`/`close`는 특정 공구 상태와 무관한 단순 물리 동작이므로 별도 API로 분리한다.
  - `open`: 해당 `layer_id`(`home_slot_row`)에 `fod_alert` 공구가 있으면 차단 — FOD 경보 서랍 개방 금지.
  - `close`: 해당 layer에 `out`/`staged` 공구가 있으면 차단 — 미반납 공구가 있는 서랍 닫기 금지.
  - 서랍 동작은 `tool_id`가 없으므로 `tool_events('rejected')` 감사 행을 남기지 않는다. 거부 사유는 `FeasibilityResult.reason`으로 전달된다.
  - `VALID_DRAWER_INTENTS = frozenset({"open", "close"})` 상수 추가. `VALID_INTENTS`("fetch"/"return")와 명시적으로 분리해 의미상 혼용을 차단한다.
  - `DBClient.check_drawer_feasibility(intent, layer_id) -> tuple[bool, str]` 추가 — `ToolRepository`에 위임 (Track C in-process 접근 경로). Track A/B ROS2 경로(`db_service_node`) 노출은 후속 작업.
