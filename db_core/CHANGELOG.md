...
- **(스키마 마이그레이션) `tools` 테이블에 `grasp_z_mm` 컬럼 추가** — `db_core/migrations/002_add_grasp_z_mm.sql` 제공(멱등 ALTER TABLE + UPDATE seed). 기존 `robot_arm.db`는 수동 적용 필요(신규 DB는 `SCHEMA_SQL` 부트스트랩에서 자동). 마이그레이션 자동 러너는 Phase 7+ 정책.
  - `ToolStatus` dataclass에 `grasp_z_mm: float | None = None` 필드 추가 (하위 호환, 기본값 None).
  - `get_tool_status()`가 `grasp_z_mm` 컬럼을 반환. 미적용 DB에서는 `OperationalError` → `except Exception` → 로그 후 실패 반환(S-2 표시 경로 유지, 조용한 열화 없음).
  - `toolbox.yaml`의 `grasp_z_mm`(tw+2mm 적용값)이 runner에서 사용하는 1차 출처. DB 컬럼의 seed 값(tw 원본)과 2mm 차이는 의도적(yaml=실제 파지 Z, DB=원본 측정). 두 출처 중 runner는 yaml 우선.
