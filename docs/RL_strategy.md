...
target_tool_id           (9,)   # ★ Goal-Conditioning: 목표 공구 One-hot (공구 9종)
```

> **[치명적 누락 보완] Goal-Conditioning**
> Task 2 / Task 3은 9종 공구 중 특정 공구를 타깃으로 해야 한다.
...
└── source/
    └── tool_transfer_bot/
        ...
        │       ├── staging_area.usd
        │       └── tools/                        # 9종 공구 개별 USD
        │           ├── driver_1.usd ~ driver_3.usd
        │           ├── wrench_1.usd ~ wrench_3.usd
        │           └── plier_1.usd ~ plier_3.usd
        ├── tasks/
        │   ├── __init__.py
        │   ├── base_env_cfg.py                   # ManagerBasedRLEnvCfg 상속
...
- 공구 9종 (드라이버 3 · 렌치 3 · 플라이어 3) 모두 커버해야 함
