...
def _create_db(path: str) -> None:
    ...
    conn.executescript(
        ...
        INSERT INTO tools VALUES (
            'spanner_16mm', '스패너 16mm', 'in_slot', 0, 0, NULL, '2026-01-01T00:00:00+00:00'
        );
        INSERT INTO tools VALUES (
            'socket_19mm', '복스 소켓 19mm', 'out', 0, 1, NULL, '2026-01-01T00:00:00+00:00'
        );
        """
    )
    conn.commit()
...
def _create_configured_tools_db(path: str) -> None:
    ...
    for index, tool_id in enumerate(CONFIGURED_TOOL_IDS):
        conn.execute(
            """
            INSERT OR REPLACE INTO tools VALUES (?, ?, 'in_slot', 0, ?, NULL, ?)
            """,
