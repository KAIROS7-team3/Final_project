...
| `watchdog_period_s` | `0.5` | heartbeat 주기 |
...
XG5000의 bit device 표기는 단순 10진수로 읽으면 안 된다. 예를 들어 `M0010`은
Modbus coil address `10`이 아니라 `0x10`, 즉 address `16`으로 매핑된다.
...
| `M0003` | `3` | P0043 시작 버튼 pulse |
| `M0010` | `16` | P0040~P0043 자기유지 reset |
| `P000` | holding register `0` | word write 테스트 |
| `P020` | holding register `0` | word read 테스트 |

`M0010`은 래더의 공통 NC reset 접점이다. `/plc_reset` 또는 `/plc_m10`에
`true`를 보내면 노드가 `M0010 ON -> pulse_duration_s 대기 -> M0010 OFF`를
수행한다.

현재 래더 이미지 기준으로 `M0100~M0103`은 watchdog이 아니라 `P0040~P0043`
자기유지 bit다. ROS2에서 직접 쓰면 출력 회로 의미가 깨질 수 있으므로 직접 제어하지
않는다. Watchdog/E-stop은 전용 래더를 추가한 뒤 별도 device로 배정해야 한다.
...
`/plc/status` QoS는 최신 상태 하나가 중요하므로 Best Effort, depth 1이다.
...
| `/plc_m0`~`/plc_m3` | `std_msgs/msg/Bool` | 대응 M coil pulse 또는 explicit OFF |
| `/plc_m_all` | `std_msgs/msg/Bool` | `M0000~M0003` batch pulse 또는 explicit OFF |
| `/plc_m10` | `std_msgs/msg/Bool` | `M0010` reset pulse 또는 explicit OFF |
| `/plc_reset` | `std_msgs/msg/Bool` | `M0010` reset pulse 또는 explicit OFF |
...
허용 값은 `idle`, `listening`, `inferring`, `moving`, `error`, `e_stop`이다.
노드는 상태 적용 전 `M0010` reset coil을 pulse해 기존 래치 출력을 끊고,
...
| 기능 | 파라미터 | 동작 |
|------|----------|------|
| Watchdog heartbeat | `enable_watchdog` | `watchdog_coil_address`에 주기적으로 `true`를 write |
| E-stop input polling | `enable_estop_poll` | `estop_input_address`가 `true`면 `e_stop` latch 후 `/plc/e_stop`에 `true` publish |

기본값은 둘 다 `false`다. 래더에서 watchdog coil과 E-stop input 주소가 확정되기
전에는 기존 P0040~P0043 bring-up에 영향을 주지 않기 위해 비활성으로 둔다.
...
```bash
ros2 topic pub --once /plc_m10 std_msgs/msg/Bool "{data: true}"
```

정상이라면 launch 터미널 로그에 `M0010 -> ON` 이후 `M0010 -> OFF`가 출력되고,
PLC 출력 `P0040~P0043`이 꺼진다.
...
- `/plc_reset` 로그가 `M0010 -> ON`, `M0010 -> OFF`로 찍히는지 확인한다.
- 로그가 `M000A` 또는 address `10` 기준처럼 보이면 설정이 오래된 것이다.
