...
# plc_core는 ROS2 패키지가 아닌 프로젝트 루트의 순수 Python 모듈이다.
# --symlink-install 환경에서는 이 파일이 소스 트리(ros2_ws/src/plc/plc/)를
# 그대로 가리키지만, 일반 install에서는 install/plc/lib/python3.x/site-packages/plc/
# 로 복사되어 깊이가 달라진다. 두 경우 모두 처리하기 위해 __file__ 기준 상위
# 디렉터리를 거슬러 올라가며 plc_core/를 포함한 디렉터리를 찾아 sys.path에
# 추가한다 (db_core를 쓰는 db 패키지와 달리 colcon package_dir 방식은
# _symlinks_in_build 정리 단계에서 실제 plc_core 소스 디렉터리를 unlink하려다
# 실패하므로 사용하지 않는다).
for _candidate in Path(__file__).resolve().parents:
    if (_candidate / "plc_core").is_dir():
