...
템플릿 경로를 바꾸고 싶으면 `gemma_prompt_template_path:=/path/to/prompt.txt`
를 넘기면 된다.
...
Gemma 없이 deterministic 파서만 올리려면 `ros2 launch voice no_gemma_voice.launch.py`
를 사용한다.
...
ros2 launch voice voice.launch.py \
  ...
  gemma_device:=auto \
  gemma_prompt_template_path:=$(ros2 pkg prefix voice)/share/voice/gemma_prompt.txt \
  toolbox_path:=$(ros2 pkg prefix voice)/share/voice/config/toolbox.yaml
```
...
ros2 run voice gemma_intent_node --ros-args \
  ...
  -p wake_words:='["코봇"]' \
  -p gemma_prompt_template_path:=$(ros2 pkg prefix voice)/share/voice/gemma_prompt.txt \
  -p toolbox_path:=$(ros2 pkg prefix voice)/share/voice/config/toolbox.yaml
```
...
- `gemma_prompt_template_path`: 기본값 `$(ros2 pkg prefix voice)/share/voice/gemma_prompt.txt`
- `toolbox_path`: 기본값 `$(ros2 pkg prefix voice)/share/voice/config/toolbox.yaml`
...
```bash
cd ros2_ws/src/voice/test/audio_samples
printf '%s\t%s\t%s\n' \
