"""No-Gemma voice intent node with DB feasibility gate.

현재 트랙은 Gemma 4 없이 deterministic parser를 사용한다. 역할은
...
전체 흐름:
    ...
    -> /db/CheckToolFeasibility service
    -> interfaces/Intent `/voice/intent`

주의: 이 노드는 DB 상태를 바꾸지 않는다. DB 상태 변경은 실제 motion 완료 후
orchestrator/motion 쪽에서 `/db/UpdateToolStatus`를 호출해야 한다.
"""
...
class RuleIntentNode(Node):
    """STT 원문을 intent 메시지로 변환하고 DB Gate로 불가능한 명령을 차단한다."""

    def __init__(self) -> None:
        super().__init__("rule_intent_node")
        # True면 "코봇 스패너 가져와"처럼 wake word로 시작하는 문장만 처리한다.
        self.declare_parameter("require_wake_word", True)

        # 현장 운영자가 부를 호출어 목록. 임시 기본값은 "코봇"이다.
        self.declare_parameter("wake_words", ["코봇"])
    ...
    def _handle_raw_text(self, message: String) -> None:
        ...
        # wake word를 통과한 시점부터는 PLC에 "명령 해석 중" 상태를 보여준다.
        self._plc_state.publish(PLC_STATE_INFERRING)

        # 현재는 Gemma 없이 deterministic keyword parser를 사용한다.
        ...
        if parsed.intent_type not in {"fetch", "return"}:
            ...
            # 이 경로는 실제 motion으로 이어지지 않으므로 바로 대기 상태로 돌린다.
            self._plc_state.publish(PLC_STATE_IDLE)
            return

        if not self._feasibility_client.wait_for_service(timeout_sec=1.0):
            self.get_logger().error(
                "/db/CheckToolFeasibility service is not available; "
                "voice intent rejected"
            )
            # DB Gate가 없으면 S-2상 명령을 진행할 수 없으므로 PLC에도 error를 알린다.
    ...
    def _handle_feasibility(self, future, parsed, raw_utterance: str) -> None:
        ...
        if not response.feasible:
            ...
            self.get_logger().warning(
                f"DB gate rejected voice intent: {response.reason}"
            )
            # 하드웨어 동작 전 단계의 거부이지만 운영자에게 보이도록 error 상태를 낸다.
            self._plc_state.publish(PLC_STATE_ERROR)
            return
        self._publish_intent(
            parsed.intent_type,
