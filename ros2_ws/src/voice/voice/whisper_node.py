...
from voice.vad import SileroVadConfig, SileroVadDetector, VadLoadError
...
class WhisperNode(Node):
    ...
    def __init__(self) -> None:
        ...
        self.declare_parameter(
            ...
            (
                "코봇, 공구함, 두산 로봇, 스테이징, 십자 드라이버, 커터칼, "
                "라쳇 렌치, 멕가이버, 스패너 16mm, 복스 소켓 19mm, 가져와, "
                "꺼내줘, 반납, 돌려놔, 취소"
            ),
        )
        ...
        self.declare_parameter("input_device_name", "")

        # RMS 기반 발화 시작 기준. 마이크 입력이 작으면 이 값을 낮춰야 한다.
        self.declare_parameter("silence_threshold", 0.02)
        self.declare_parameter("min_speech_chunks", 4)

        # Silero VAD를 사용해 RMS를 통과한 소리가 실제 사람 음성인지 재검증한다.
        ...
        self.declare_parameter("vad_min_silence_ms", 100)
        self.declare_parameter("vad_speech_pad_ms", 100)
        ...
        self._plc_state = VoicePlcStatePublisher(self)
    ...
    def publish_transcript(self, transcript: str) -> bool:
        ...
        message = String()
        message.data = text
        self._plc_state.publish(PLC_STATE_LISTENING)
    ...
    def _start_microphone_worker(self) -> None:
        ...
        try:
            enable_vad = (
                self.get_parameter("enable_vad")
                .get_parameter_value()
                .bool_value
            )
            speech_detector = None
            if enable_vad:
                speech_detector = SileroVadDetector(
                    SileroVadConfig(
                        threshold=self.get_parameter("vad_threshold")
                        .get_parameter_value()
                        .double_value,
                        min_speech_duration_ms=self.get_parameter(
                            "vad_min_speech_ms"
                        )
                        .get_parameter_value()
                        .integer_value,
                        min_silence_duration_ms=self.get_parameter(
                            "vad_min_silence_ms"
                        )
                        .get_parameter_value()
                        .integer_value,
                        speech_pad_ms=self.get_parameter("vad_speech_pad_ms")
                        .get_parameter_value()
                        .integer_value,
                    )
                )
            ...
            self._recorder = MicRecorder(
                MicRecorderConfig(
                    ...
                    .get_parameter_value()
                    .double_value,
                    min_speech_chunks=self.get_parameter("min_speech_chunks")
                    .get_parameter_value()
                    .integer_value,
                    enable_vad=enable_vad,
                    input_device_index=self.get_parameter("input_device_index")
                    .get_parameter_value()
                    .integer_value,
                    input_device_name=self.get_parameter("input_device_name")
                    .get_parameter_value()
                    .string_value,
                ),
        ...
        except (AudioInputError, WhisperLoadError, VadLoadError) as exc:
